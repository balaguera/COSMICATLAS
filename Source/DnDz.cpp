...
void DnDz::dndz(void *p, vector<gsl_real> &new_zz_v, vector<gsl_real>&new_dndz_v, vector< vector<gsl_real> > &new_dndz_matrix)
{
  ...
  vector<real_prec >prop = s_dndz_data->properties;
  ...
  int n_objects = prop.size() / n_columns;
  
  if(s_dndz_data->compute_dndz){



    for(int i=0;i<n_dndz;i++)zz_v[i]=redshift_min_sample+(i+0.5)*Delta_Z;
    ...
    for(int i=0;i<n_dndz;i++){
      ...
      real_prec Volumen=   (area_survey*M_PI/180)*(pow(rmax,3)-pow(rmin,3))/3.;
      dndz_v[i]=dndz_v[i]*(s_dndz_data->alpha0)/Volumen;  //do not divide by Delta Z. 
    } 
    //    cout<<RED<<"TEST GOING ON HERE, 2P_STATISTICS"<<endl;
    /*Smooth the histogram with a cubic spline*/
    //gsl_bspline(zz_v, dndz_v, new_zz_v, new_dndz_v);
    
    // Pass the same historgram without smooth at all, 
    new_zz_v=zz_v;
    new_dndz_v=dndz_v;
    ...
    if(!s_dndz_data->constant_depth){
      ...
      real_prec Delta_Z=(redshift_max_sample-redshift_min_sample)/n_dndz;
      for(int i=0;i<n_dndz;i++)zz_v[i]=redshift_min_sample+(i+0.5)*Delta_Z;
      for(int i=0;i<n_dndz;i++)for(int j=0;j<npixels;j++)dndz_v[i][j]=0;
#ifdef HEALPIX
      for(int i=0;i<n_objects;i++){
	...
	for(int i=0;i<new_dndz_matrix.size();i++)new_dndz_matrix[i][j]=(sys_of_coord_r== 0 ? 1.0 : new_dndz_v[i]); 
      }
    }
  }
}
