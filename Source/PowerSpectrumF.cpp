...
//        for(int i=0;i<vzz.size();++i)cout<<vzz[i]<<"  "<<vrc[i]<<endl;
        // **************************************************************************
        ...
        if(true==this->params._use_random_catalog() && true==params._nbar_tabulated())
            z_v.resize(params._new_N_dndz_bins(),0);
        // *************************************************************************
        ...
        if(this->params._sys_of_coord_g()>0)
          {
            So.message_screen("Transform to cartesian coordinates in tracer catalogue ");
                    fftw_functions.cart_coordinates(&s_data_struct_g);
            So.DONE();
        }
        if(true==this->params._use_random_catalog())
          {
            ...
            So.message_screen("Transform to cartesian coordinates in random catalogue ");
                        fftw_functions.cart_coordinates(&s_data_struct_r);
                So.DONE();
              }
          }
        // *****************************************************************************************
        ...
        if(true==this->params._use_random_catalog())
          {
            ...
            fftw_functions.get_parameters_estimator(verbose); // IMPORTANT, COMPUTE ALPHA, NORMALIZATION ETC
        ...
        } // CLOSES if this is catalot or field
...
// This function is to be read from the cosmicatlas.cpp main function: verbose is to show detailed information, mcut is to force the analysis to have a minium cut in a given property
  // depending on w
  // This method can be accesed from compilation with the -w option


  // WE need to tell the method to add_catalog,
  // and build a method like mass assignmenbt (or direcly here, loops over rans and catalog
  // to get the values of alpha, normalization, etc, that the window needs

   // Ojo, que quiz{as sea mejor medior el eszpecto y llamar a la ge_wqindow_multipole
...
// This computation involves a double sum over the random catalog.

  // ------------------------------------------------------------------------
#ifdef _FULL_VERBOSE_
  this->So.message_screen("\tAllocating weights randoms");
#endif
...
#pragma omp parlalel for
  ...
  this->alpha=1;//fftw_functions._alpha();
  this->normal_power=1;//fftw_functions._normal_power();
  So.DONE();
  // ------------------------------------------------------------------------
...
#pragma omp parallel for
  ...
  cout<<this->params._d_kmin()<<"  "<<this->params._d_kmax()<<endl;
  ...
  struct pair_info{
    ULONG ipair;
    ULONG jpair;
    real_prec weight_pair;
    real_prec Rminus;
    real_prec DeltaR;
    vector<real_prec> Legpol;
  };
...
#pragma omp parallel for
  ...
  for (int l=0;l<MAXIMUM_MULTIPOLE_POWER;++l)              // Loop over multipole l
    {
       ...
       for (int lp=0;lp<MAXIMUM_MULTIPOLE_WINDOW_MATRIX;++lp)              //Loop over multipole lp
	    {
          ...
          {
            real_prec factor_n=(2*l+1)*pre_factor*kvector_data_GL[jk]*kvector_data_GL[jk]*weight_GL[jk];
            real_prec window=0;
#ifdef _USE_OMP_
...
//                cout<<this->kvector_data[ik]<<"  "<<kvector_data_GL[jk]<<"   "<<pairs[ip].Rminus<<"  "<<bess_l<<"  "<<bess_lp<<endl;
           }
   ...
   for(ULONG itr=0;itr<tracer_cat.size();++itr)
     {
       ...
       for(ULONG i=initial_mode; i< new_Nft/2;++i)
        ...
        // Here the fist vol comes from the discrete FT. The term vol/Ngrid is the normzalization of the ter 1/dm, i.e the DM power is normalized with Ngrid/vol
   ...
   for(ULONG itr=0;itr<tracer_cat.size();++itr)
     {
       ...
       for(ULONG i=Kmin_bin; i< new_Nft/2;++i)
        ...
        for(ULONG i=Kmin_bin; i< power_cross.size();++i)
          {
            // *** here it must be nmodes*(<power>_av)= nmodes*(power/nmodes)=power. That's why I do not use here "nmodes".
            // The first factor in the numerator has *Ngrid to get the right normzalization
