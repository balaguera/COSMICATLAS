...
gsl_real DensityProfiles::idensity_rc(gsl_real c, void *p){
  ...
  */
  ...
  real_prec dcontrast=Cf.density_contrast_top_hat(z,p);
  real_prec deltac=Cf.critical_density(z,p);
  
  string prof=s_cp->density_profile;

  real_prec rvir,rs,alpha,f,nu;
  rvir=pow((real_prec)(3.*M/(4.*M_PI*(dcontrast)*Cf.mean_matter_density(z,p))),(real_prec)1./3.);
  
  if(prof=="nfw"){ 
    f=(1./3.)*(dcontrast)*(Cf.mean_matter_density(z,p))*pow(c,3)/(log(1+c)-(c/(1.+c)));
...
real_prec DensityProfiles::density_kc(real_prec k, real_prec M, real_prec z,  void *cp){
  ...
  Cosmology Cf;
  struct s_CosmologicalParameters * s_cp= (struct s_CosmologicalParameters *)cp;
  real_prec dcontrast=Cf.density_contrast_top_hat(z,cp);
  real_prec rvir=pow((real_prec)(3.*M/(4.*M_PI*(dcontrast)*Cf.mean_matter_density(z,cp))),(real_prec)1./3.);
...
void DensityProfiles::nfw_parameters(real_prec M, real_prec z, real_prec *fc, real_prec *r_vir,real_prec *c,real_prec *rhos,real_prec *rs, void *cp){
  ...
  *fc   = log(1+(*c))-((*c)/(1.+(*c)));
  ...
  real_prec density_contrast_top_hat=Cf.density_contrast_top_hat(z,cp);
  real_prec mean_mat_den=Cf.mean_matter_density(z,cp);
